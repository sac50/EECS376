...
//		{
		psi_robot=tf::getYaw(last_map_pose.pose.orientation);//get direction
	...
	//	if (eroded.at<cv::Vec3b>(i,j)[0] = 255)
			{
				...
				if (p.x > 0.1 || p.y > 0.1  || p.z > 0.1) {
					ROS_INFO_STREAM(boost::format("Pixel (%d, %d) maps to 3D point (%.2f, %.2f, %.2f) in TF frame = \"%s\"")
						...
						ROS_INFO_STREAM(boost::format("Pixel (%d, %d) maps to 3D point (%.2f, %.2f, %.2f) in Robot frame = \"%s\"")
						%i %j %x_r %y_r %z_r %cloud_msg->header.frame_id);
						...
						ROS_INFO_STREAM(boost::format("Pixel (%d, %d) maps to 3D point (%.2f, %.2f, %.2f) in Map frame = \"%s\"")
						%i %j %x_m %y_m %z_m %cloud_msg->header.frame_id);
