...
double Kd=0,K_omega=7.5,omega_sat=2.0;
double a_max = 0.25;
bool estop;
bool stopped;
bool braking=false;
...
geometry_msgs::Twist getVelocity(double time, double v_past, double v_cmd, double segDistDone, double segLength, double segType,Path path){
	...
	double psi_des,psi_err,psi_robot=tf::getYaw(last_map_pose.pose.orientation);
	Vector xy(last_map_pose.pose.position.x,last_map_pose.pose.position.y);
	double d = path.d_off(xy);
	...
	double psidot = omega_cmd;
	double xdot = cos(psi_robot);
	double ydot = sin(psi_robot);
...
//	ROS_INFO("V-CMD == %f", v_cmd);
	...
	if(segType==1){	
		...
		velocity_cmd.angular.z = omega_cmd;
		return velocity_cmd;
	}
	else if(segType==2){
...
int main(int argc,char **argv)
{
	...
	Vector ignore = Segment.n_hatCalc();
