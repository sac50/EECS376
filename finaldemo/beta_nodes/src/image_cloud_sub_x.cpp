	...
	const sensor_msgs::CameraInfo::ConstPtr& cam_msg)
{
	...
	double red_mid_x = 0.0;
	double red_mid_y = 0.0;
	double red_mid_c = 0.0;
	
	double blue_left_x = 0.0; // keep track of the leftmost and rightmost point that is on the floor
	double blue_left_y = 0.0;
	double blue_right_x = 0.0;
	double blue_right_y = 0.0;
	double x_sum = 0.0;
	double y_sum = 0.0;
	double x_avg = 0.0;
	double y_avg = 0.0;
	...
	for (int i = lb; i > ub; i-=STEP) {//rows
		for (int j = mp; j > lp; j-=STEP) {//cols left
...
//				std::cout << "\tM:(" << map_point.point.x << ", " << map_point.point.y << ", " << map_point.point.z << ")\n";
				
				if (map_point.point.z < 0.03) { // continuously update the leftmost "non-obstacle" point
					blue_left_x = map_point.point.x;
					blue_left_y = map_point.point.y;
				}
				else {	// and once we find an obstacle, mark it on the screen and go to the next row
					cv::circle(image, cv::Point(j, i), STEP, CV_RGB(0, 0, 250), 3);
					break;
				}
			}
		} //col (left side)

		for (int j = mp; j < rp; j+=STEP) { //right side
			...
			if (p.x > 0.01 || p.y > 0.01  || p.z > 0.01) {
				...
				if (map_point.point.z < 0.03) {// if its the floor
					blue_right_x = map_point.point.x;
					blue_right_y = map_point.point.y;
				}
		...
		double blue_x_midpoint = (blue_left_x + blue_right_x)/2;
		double blue_y_midpoint = (blue_left_y + blue_right_y)/2;

		double weight = Kw*(1.0/((double)counter+1.0)); //(constant * (299 - (something that counts up to 299)))

		x_sum += blue_x_midpoint*weight; //should weight this by distance from robot
		y_sum += blue_y_midpoint*weight;

		ROS_INFO_STREAM("R: " << i << " X: " << blue_x_midpoint << " Y: " << blue_y_midpoint
									<<" W: " << weight);

		//ROS_INFO_STREAM("RED C: " << red_c);
		//ROS_INFO_STREAM("MULTI_COUNTER: " << multi_counter);
		//ROS_INFO_STREAM("MULTIPLIER: " << multiplier);
		//double r_avg_x = red_x / red_c;
		//double r_avg_y = red_y / red_c;
		//red_mid_x += r_avg_x * (299 - multi_counter) * multiplier; // becomes -nan
		//red_mid_y += r_avg_y * (299 - multi_counter) * multiplier;
		//ROS_INFO_STREAM("midpoint: " << red_mid_x << " " << red_mid_y);
		//red_mid_c += (299-multi_counter);
		////red_x = 0.0;
		//red_y = 0.0;
		//red_c = 0.0;
		//multi_counter+=1.0;
	...
	ROS_INFO_STREAM("Xsum: " << x_sum << " Ysum: " << y_sum);

	//int lazysum = (((299*299)/2)+299/2);
	ROS_INFO_STREAM("divisor: " << Kw); //lazysum);

	x_avg = x_sum / Kw; // lazysum;
	y_avg = y_sum / Kw; // lazysum;
	
	ROS_INFO_STREAM("Xavg: " << x_avg << " Yavg: " << y_avg);


	point.point.x = x_avg;
	point.point.y = y_avg;
	point.point.z = 0.0;
	//ROS_INFO_STREAM(boost::format("Published Point (%f, %f") %red_avg_x %red_avg_y);
	pub.publish(point);
