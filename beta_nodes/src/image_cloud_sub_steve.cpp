	...
	const sensor_msgs::CameraInfo::ConstPtr& cam_msg)
{
	...
	for (int i = 1; i < eroded.rows-STEP; i+=STEP) 
	{
			for (int j = 1; j < eroded.cols-STEP; j+=STEP) 
			{
		//	if (eroded.at<uchar>(i,j) == 255)
			{
				...
				if (p.x > 0.1 || p.y > 0.1  || p.z > 0.1) //filter nans
				{
					...
					try
					{
						...
						//tfl->transformPoint("base_link", kinect_point, map_point);
						//map_point.point.z-=2;
					}
...
/*
					if (z_m < 0.1) {
						...
						cv::circle(image, cv::Point(j, i), STEP, CV_RGB(0, 0, 0), 3);	
					} else {
						cv::circle(image, cv::Point(j, i), STEP, CV_RGB(250, 250,250), 3);	
					}
*/

					double roundx = floor(x_m*10)/10.0;
					double roundy = floor(y_m*10)/10.0;
	/*					
                    bool addToList = true;
                    for (int vec_i = 0; vec_i < greenlist.size(); vec_i++)
                    {
                        double gX = greenlist.at(vec_i).point.x;
                        double gY = greenlist.at(vec_i).point.y;
                        if (fabs(roundx-gX)<.01 && fabs(roundy-gY)<.01)
                        {
							std::cout << "Disqualified in round 1\n";
							addToList = false;
                        }
                    }
                    /*
					if(fabs(z_m)>0.2)
					{
						std::cout << "Disqualified in round 2!!!\n";
                        addToList=false;
                    }
					
                    if (true) {
                        //geometry_msgs::PointStamped point;
                        //point.point.x = roundx;
                        //.point.y = roundy;
						//point.point.z = 0.0;
						//point.header = kinect_point.header;
						//.header.frame_id = "map";
                        greenlist.push_back(map_point);
						//ROS_INFO_STREAM(boost::format("chunky orange at map x:%.2f y:%.2f z:%.2f")
...
*/
	...
	//ROS_INFO_STREAM(boost::format("Green Vector Size: %d") %greenlist.size());
	//ROS_INFO_STREAM("Green list:\n");
	for (int vec_i = 0; vec_i < greenlist.size(); vec_i++)
	{
		pub.publish(greenlist.at(vec_i));
		//ROS_INFO_STREAM(boost::format("x:%f, y:%f, z:%f") %greenlist.at(vec_i).point.x  %greenlist.at(vec_i).point.y %greenlist.at(vec_i).point.z);
	}
	//ROS_INFO_STREAM(boost::format("Got %d orange pixels") %count);																
